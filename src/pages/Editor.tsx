...
export default function Editor() {
  ...
  return (
    <div className="flex-1 flex flex-row h-full overflow-hidden text-on-surface bg-surface relative">
      ...
      <aside className="w-72 bg-[#131b2e] flex flex-col no-scrollbar overflow-y-auto px-6 py-8 space-y-6 flex-shrink-0">
        <div className="px-6 mb-4 flex justify-between items-center">
          ...
          <button 
            ...
            className="text-slate-400 hover:text-[#c0c1ff] transition-colors"
            title="Add Chapter"
          >
                  ...
                  <div className="flex justify-between items-center group/chapter px-2 mb-3">
                    {editingId === chapter.id ? (
                      <input 
                        ...
                        className="bg-transparent border-b border-[#c0c1ff] outline-none text-[#dae2fd] text-xs font-bold uppercase tracking-widest w-full"
                      />
                    ) : (
                      <h3 
                        onDoubleClick={() => startEdit(chapter.id, chapter.title)}
                        className="text-xs font-bold text-[#dae2fd] uppercase tracking-widest cursor-text select-none"
                      >
                        {chapter.title}
                      </h3>
                    )}
                    <div className="flex gap-1 opacity-0 group-hover/chapter:opacity-100 transition-opacity">
                      <button onClick={() => activeProjectId && createScene(chapter.id, activeProjectId)} className="text-slate-400 hover:text-[#c0c1ff]">
                    ...
                    {chapterScenes.map(scene => (
                      <div
                        ...
                        className={`py-2 px-3 rounded-lg text-xs cursor-pointer transition-colors flex justify-between items-center group/scene ${activeSceneId === scene.id
                            ? 'bg-[#171f33] text-[#c0c1ff] border-l-2 border-[#c0c1ff]'
        ...
        <div className="pt-8 border-t border-[#464554]/10">
          ...
          <div className="space-y-1">
            {['delete', 'folder'].map((icon, i) => (
              <div key={icon} className="flex items-center gap-3 py-2 px-3 text-slate-400 text-xs hover:bg-[#222a3d] rounded-lg cursor-pointer transition-colors">
      ...
      {/* Main Canvas */}
      <main className="flex-1 flex flex-col bg-[#0b1326] relative">

        {/* Editor area */}
        <div className="flex-1 flex overflow-hidden">
          {!activeScene ? (
            <section className="flex-1 bg-[#171f33] flex flex-col items-center justify-center overflow-y-auto no-scrollbar">
              <div className="text-center">
                 ...
                 <p className="font-label text-xs uppercase tracking-widest text-[#c0c1ff]/50 mt-2">Create a chapter and scene in the sidebar to begin.</p>
              </div>
            </section>
          ) : (
<>
          {/* Manuscript / Zen Editor */}
          <section className="flex-1 bg-[#171f33] flex flex-col items-center overflow-y-auto no-scrollbar">
            <div className="max-w-3xl w-full px-12 py-24 min-h-screen">
              <div className="mb-12 space-y-2">
                <span className="text-[11px] font-headline uppercase tracking-[0.3em] text-[#c0c1ff]/60">
                  {projectChapters.find(c => c.id === activeScene.chapter_id)?.title}
                </span>
                <h2 className="text-4xl font-body italic text-[#dae2fd] leading-tight">{activeScene.title}</h2>
                <div className="flex items-center gap-4 pt-4">
                  <span className="text-[10px] font-headline text-slate-500 border border-[#464554]/30 px-2 py-0.5 rounded-full uppercase tracking-tighter">
              ...
              <textarea
                className="drop-cap font-body text-xl leading-relaxed text-[#dae2fd]/90 w-full bg-transparent border-none outline-none resize-none"
          ...
          <aside className="w-80 bg-[#131b2e] border-l border-[#464554]/5 flex flex-col flex-shrink-0">
            <div className="p-8 space-y-8">
              ...
              <div>
                ...
                <div className="bg-[#171f33] p-4 rounded-xl space-y-3">
                  <textarea
                    placeholder="Capture atmospheric goals or hidden themes here..."
                    className="w-full bg-transparent text-xs text-[#c7c4d7] leading-relaxed font-body resize-none outline-none"
                    rows={4}
                ...
                <div className="space-y-4 max-h-40 overflow-y-auto no-scrollbar">
                  {projectCharacters.length === 0 ? (
                    ...
                    <div key={char.id} className="flex items-center gap-3 group cursor-pointer">
                       <div className="w-10 h-10 rounded-xl bg-[#222a3d] border border-[#464554]/30 overflow-hidden shrink-0">
                      ...
                      <div className="truncate">
                        <p className="text-sm font-bold text-[#dae2fd] truncate">{char.name}</p>
              ...
              {/* Writing Goal */}
              <div className="pt-6">
                <div className="bg-gradient-to-br from-[#222a3d] to-[#131b2e] p-6 rounded-2xl border border-[#464554]/10">
                  <p className="text-[10px] uppercase tracking-widest text-[#c0c1ff] font-bold mb-2 font-headline">Writing Goal</p>
                  <div className="flex items-end gap-2 mb-3">
                    <span className="text-2xl font-bold text-[#dae2fd]">{wordCount.toLocaleString()}</span>
                  ...
                  <div className="w-full bg-[#060e20] h-1.5 rounded-full overflow-hidden">
                    <motion.div
                      ...
                      style={{ background: '#c0c1ff', boxShadow: '0 0 8px rgba(192,193,255,0.4)' }}
            ...
            {/* Bottom actions */}
            <div className="mt-auto p-8 border-t border-[#464554]/5">
              <div className="flex items-center justify-between text-slate-500">
                <button title="Export Manuscript (.txt)" onClick={exportManuscript} className="material-symbols-outlined cursor-pointer hover:text-[#c0c1ff] text-[#dae2fd] transition-colors bg-[#222a3d] p-2 rounded-xl border border-[#464554]/30">
                  download
                </button>
                <button title="View History" className="material-symbols-outlined cursor-pointer hover:text-[#ffb783] transition-colors p-2">
                  history
                </button>
                <button title="More Options" className="material-symbols-outlined cursor-pointer hover:text-[#c0c1ff] transition-colors p-2">
                  more_vert
                </button>
              </div>
            </div>
