        ...
        {isModalOpen && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-3xl font-body font-bold text-[#dae2fd] mb-6">Archive New Lore</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={3}
                    className="w-full bg-[#0b1326] border border-[#464554]/30 rounded-xl px-4 py-3 text-[#dae2fd] outline-none focus:border-[#c0c1ff]/50 transition-colors resize-none"
                  />
                </div>
      ...
      <div className="page-content space-y-12 max-w-7xl mx-auto w-full relative z-20">
        ...
        <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <motion.div
            ...
            className="lg:col-span-2 relative group overflow-hidden rounded-[2rem] bg-[#171f33]" style={{ height: 400 }}
          >
            <img
            ...
            />
            <div className="absolute inset-0 bg-gradient-to-t from-[#171f33] via-[#171f33]/20 to-transparent" />
            <div className="absolute bottom-0 left-0 p-10 space-y-4">
              <span className="px-4 py-1.5 rounded-full bg-[#4f319c] text-[#bea8ff] text-xs font-bold uppercase tracking-widest">Active Schema</span>
              <h3 className="text-4xl font-body italic text-[#dae2fd]">{activeProject.title} Framework</h3>
              <p className="text-[#c7c4d7] max-w-lg font-body text-lg leading-relaxed">{projectItems.length} logical entities have been mapped inside this universe.</p>
              <div className="flex gap-4 pt-2">
                <button onClick={() => setIsModalOpen(true)} className="bg-[#c0c1ff] px-6 py-2.5 rounded-xl text-[#1000a9] font-bold text-sm">Add Metadata</button>
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="bg-[#131b2e] rounded-[2rem] p-8 flex flex-col"
          >
            ...
            <div className="space-y-6 flex-grow">
              <div className="p-4 rounded-2xl bg-[#222a3d] border-l-2 border-[#c0c1ff]/40">
                <p className="text-xs text-slate-500 mb-1 font-headline uppercase">Daily AI Prompt</p>
                <p className="text-[#dae2fd] font-body italic text-lg leading-snug">"What are the consequences of defying the natural laws of your new location?"</p>
              </div>
            </div>
          </motion.div>
        </section>
        ...
        <section className="space-y-8">
            ...
            <div className="flex flex-wrap gap-4">
              {filters.map(f => (
                <button key={f} onClick={() => setActiveFilter(f)}
                  className={`px-6 py-2 rounded-full font-bold text-sm transition-all ${activeFilter === f ? 'bg-[#c0c1ff] text-[#1000a9]' : 'bg-[#131b2e] text-[#c7c4d7] hover:bg-[#171f33]'}`}>
                  {f}
                </button>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
            ...
            <motion.div
              ...
              className="bg-[#131b2e] rounded-[2rem] p-8 border-2 border-dashed border-[#464554]/30 flex flex-col items-center justify-center cursor-pointer hover:border-[#c0c1ff]/40 transition-all group min-h-[300px]"
            >
              <div className="w-16 h-16 rounded-full bg-[#171f33] flex items-center justify-center mb-4 group-hover:bg-[#8083ff]/20 transition-colors">
                <span className="material-symbols-outlined text-slate-500 group-hover:text-[#c0c1ff] transition-colors">note_add</span>
              </div>
              <p className="font-headline text-[11px] uppercase tracking-widest text-slate-500 group-hover:text-[#dae2fd] transition-colors">Deposit Archive Entry</p>
            </motion.div>
              ...
              if (pattern === 0) {
                 ...
                 return (
                  <motion.div
                    ...
                    className="bg-[#171f33] rounded-[2rem] group relative h-fit"
                  >
                    ...
                    <div className="overflow-hidden rounded-[2rem]">
                      ...
                      <div className="p-8 space-y-4">
                        <div className="flex justify-between items-center mt-[-2rem] relative z-10">
                          <span className="px-3 py-1 rounded-full bg-[#4f319c]/30 text-[#bea8ff] text-[10px] font-bold uppercase backdrop-blur-md">{item.category}</span>
                        </div>
                        <h4 className="text-2xl font-body text-[#dae2fd]">{item.name}</h4>
                        <p className="text-[#c7c4d7] font-body text-lg leading-relaxed">{item.description}</p>
                      </div>
                    </div>
                  </motion.div>
                 );
                 ...
                 return (
                  <motion.div
                    key={item.id}
                    initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }}
                    className="bg-[#171f33] border-l-4 border-[#c0c1ff] rounded-[2rem] p-8 space-y-6 cursor-pointer group relative h-fit"
                  >
                    ...
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <span className="text-[10px] font-bold text-[#c0c1ff] tracking-[0.2em] uppercase">{item.category}</span>
                      </div>
                      <h4 className="text-2xl font-body text-[#dae2fd] mb-4">{item.name}</h4>
                      <p className="text-[#c7c4d7] font-body text-lg leading-relaxed mb-4">{item.description}</p>
                      <div className="p-4 rounded-xl bg-[#060e20] text-xs text-slate-400 font-headline">
                        <span className="font-bold text-[#c0c1ff] block mb-1">STORY IMPACT: HIGH</span>
                 ...
                 return (
                  <motion.div
                    key={item.id}
                    initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.06 }}
                    className="bg-[#131b2e] rounded-[2rem] p-8 space-y-6 cursor-pointer group relative h-fit"
                  >
                    <button onClick={(e) => { e.stopPropagation(); deleteWorldItem(item.id); }} className="absolute z-20 right-4 top-4 p-2 text-slate-400 hover:text-red-400 opacity-0 group-hover:opacity-100">
                      <span className="material-symbols-outlined text-sm">delete</span>
                    </button>
                    <div>
                      <div className="flex justify-between items-start mb-4">
                        <span className="p-3 rounded-2xl bg-[#222a3d] text-[#c0c1ff]">
                      ...
                      <h4 className="text-2xl font-body group-hover:text-[#c0c1ff] transition-colors text-[#dae2fd]">{item.name}</h4>
                      <p className="text-[#c7c4d7] font-body text-lg leading-relaxed">{item.description}</p>
                    </div>
                  </motion.div>
                 );
