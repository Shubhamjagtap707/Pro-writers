      ...
      <div className="flex-grow relative overflow-hidden flex z-10">
        ...
        <div className="flex-grow relative p-12 overflow-hidden">
          <svg className="absolute inset-0 w-full h-full pointer-events-none z-0">
            ...
            {/* Silas → Elara (ally) */}
            <path className="opacity-30" d="M 400 300 Q 300 450 250 550" fill="none" stroke="#c0c1ff" strokeWidth="2" />
            {/* Silver Man → Elara (conflict) */}
            <path className="opacity-20" d="M 700 350 Q 500 500 250 550" fill="none" stroke="#ffb783" strokeWidth="2" />
            <defs>
              <linearGradient id="grad-antag" x1="0%" x2="100%" y1="0%" y2="0%">
                <stop offset="0%" style={{ stopColor: '#c0c1ff', stopOpacity: 1 }} />
                <stop offset="100%" style={{ stopColor: '#ffb4ab', stopOpacity: 1 }} />
              </linearGradient>
            </defs>
          </svg>
          ...
          <div className="absolute z-10 group cursor-pointer" style={{ top: 260, left: 340 }} onClick={() => setActiveNode('silas')}>
            <div className="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-[#c0c1ff] to-[#8083ff]"
              style={{ boxShadow: '0 0 30px rgba(192,193,255,0.3)' }}>
              <div className="w-full h-full rounded-full overflow-hidden border-4 border-[#0b1326]">
            ...
            <div className="mt-4 text-center">
              <span className="block text-[#c0c1ff] font-bold tracking-tight">Silas Thorne</span>
              <span className="text-[10px] uppercase tracking-tighter text-slate-400">Protagonist</span>
            </div>
            <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-3 h-3 bg-[#c0c1ff] rounded-full border-2 border-[#0b1326] shadow-lg" />
          </div>
          ...
          <div className="absolute z-10 group cursor-pointer" style={{ top: 310, left: 680 }} onClick={() => setActiveNode('silver')}>
            <div className="w-24 h-24 rounded-full p-1 bg-[#2d3449] group-hover:scale-105 transition-transform"
              style={{ boxShadow: '0 0 20px rgba(192,193,255,0.15)' }}>
              <div className="w-full h-full rounded-full overflow-hidden border-2 border-[#0b1326] bg-[#131b2e]">
            ...
            <div className="mt-3 text-center">
              <span className="block text-[#ffb4ab] font-semibold text-sm">The Silver Man</span>
          ...
          <div className="absolute z-10 group cursor-pointer" style={{ top: 520, left: 180 }} onClick={() => setActiveNode('elara')}>
            <div className="w-20 h-20 rounded-full p-1 bg-[#222a3d] group-hover:scale-105 transition-transform"
              style={{ boxShadow: '0 0 20px rgba(192,193,255,0.15)' }}>
              <div className="w-full h-full rounded-full overflow-hidden border-2 border-[#0b1326]">
            ...
            <div className="mt-2 text-center">
              <span className="block text-[#cebdff] font-semibold text-sm">Elara Vance</span>
          ...
          <div className="absolute bottom-8 left-12 glass-panel p-4 rounded-xl flex gap-6 items-center border border-white/5">
            {[
              { color: '#c0c1ff', solid: true, label: 'Ally' },
              { color: '#ffb4ab', solid: false, label: 'Conflict' },
              { color: '#ffb783', solid: true, label: 'Family' },
            ].map(item => (
        ...
        <aside className="w-96 h-full glass-panel z-20 flex flex-col p-8 border-l border-white/5 flex-shrink-0">
          <div className="mb-8">
            <div className="flex justify-between items-start mb-6">
              <div>
                <span className="text-[#c0c1ff] text-[10px] font-bold uppercase tracking-[0.2em] mb-1 block">Active Focus</span>
                <h2 className="font-body text-4xl text-[#dae2fd] leading-tight">Silas Thorne</h2>
              </div>
              <button className="p-2 rounded-full bg-[#222a3d] text-slate-400 hover:text-white transition-colors">
                <span className="material-symbols-outlined">edit_note</span>
              </button>
            </div>
            <div className="flex gap-2 flex-wrap mb-6">
              {['Disgraced Detective', 'Obsessive', '38 Years Old'].map(tag => (
                <span key={tag} className="bg-[tag === 'Disgraced Detective' ? '#4f319c' : '#2d3449'] text-[10px] px-3 py-1 rounded-full uppercase font-bold text-slate-300"
                  style={{ background: tag === 'Disgraced Detective' ? '#4f319c' : '#2d3449', color: tag === 'Disgraced Detective' ? '#bea8ff' : undefined }}
                >{tag}</span>
              ))}
            </div>
          ...
          <div className="space-y-6 overflow-y-auto pr-2 no-scrollbar">
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  { icon: 'swords', name: 'The Silver Man', rel: 'Antagonistic • Linked by the 1892 Murders', iconBg: 'bg-[#93000a]/20', iconColor: 'text-[#ffb4ab]' },
                  { icon: 'favorite', name: 'Elara Vance', rel: 'Deep Ally • Emotional Anchor', iconBg: 'bg-[#8083ff]/20', iconColor: 'text-[#c0c1ff]', filled: true },
                ].map(conn => (
                  <div key={conn.name} className="flex items-center gap-4 p-3 rounded-xl bg-[#131b2e] hover:bg-[#171f33] transition-colors group cursor-pointer">
                    ...
                    <div>
                      <p className="text-sm font-bold text-[#dae2fd]">{conn.name}</p>
            ...
            <div>
              ...
              <div className="border-l-2 border-[#464554]/20 ml-2 pl-6 space-y-6 py-2">
                {[
                  ...
                  <div key={arc.range} className="relative">
                    <div className={`absolute -left-[31px] top-1 w-2 h-2 rounded-full ring-4 ring-[#0b1326] ${arc.active ? 'bg-[#c0c1ff]' : 'bg-slate-600'}`} />
                    <p className={`text-[11px] font-bold mb-1 ${arc.active ? 'text-[#c0c1ff]' : 'text-slate-500'}`}>{arc.range}</p>
          ...
          <div className="mt-auto pt-6">
            <button className="w-full flex items-center justify-between p-4 rounded-xl bg-[#ffb783]/10 text-[#ffb783] hover:bg-[#ffb783]/20 transition-colors border border-[#ffb783]/20">
